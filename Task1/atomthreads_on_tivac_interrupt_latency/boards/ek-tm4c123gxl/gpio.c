...
void setup_led(void)
{
    ...
    //NVIC interrupt configuration
           NVIC_EN0_R=0x40000000; //enable port F interrupt in NVIC
           NVIC_PRI7_R = (NVIC_PRI7_R | 0xFFFFFFFF) & (0xFF1FFFFF);// priority 5

        //GPIO interrupt configuration
        GPIO_PORTF_IM_R=0x00; //interrupt disable (masking)
        GPIO_PORTF_IS_R =0x00;  //edge sensitive interrupt
        GPIO_PORTF_IEV_R=0x00; //falling edge sensitive
        GPIO_PORTF_IBE_R=0x00; //---
        GPIO_PORTF_ICR_R  =0x11; //clear the interrupt if any
        GPIO_PORTF_IM_R=0x11;  //interrupt enable


    /* configuring portD pin 1 PD0 as interrupt */
    /* configuring port D pin2 PD1 as output pin : interrupt to BBB*/
    ...
    /* set priority for pin PD1 : priority level 2 */
    ...
    /* Both edge senistive : no */

        GPIO_PORTD_IBE_R=0x00;    // 0x00
