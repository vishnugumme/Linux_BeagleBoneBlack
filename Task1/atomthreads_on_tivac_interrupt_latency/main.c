...
static ATOM_TCB main_tcb,task_1;
...
static uint8_t task_1_stack[MAIN_STACK_SIZE_BYTES];
...
static void task_1_func(uint32_t data);
...
int main(void)
{
    ...
    setup_board();
    status = atomSemCreate(&SEM1,1);
    status = atomSemCreate(&SEM2,0);
    if(status == ATOM_OK) {
    ...
    if(status == ATOM_OK) {
        ...
        status = atomThreadCreate( &task_1,
                                   task_1_PRIO,
                                   task_1_func,
                                   0,
                                   &task_1_stack[0],
...
static void main_thread_func(uint32_t data __maybe_unused)
{
    ...
    while(1) {
        ...
        atomTimerDelay( MS_TO_TICKS(100) );
        }
...
static void task_1_func(uint32_t data __maybe_unused)
{
...
void gpiof_isr(void)
{
    ...
    GPIO_PORTF_ICR_R = 0x11;     /* clear PF0 int */
